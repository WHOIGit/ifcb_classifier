...
class SaveValidationResults(ptl.callbacks.base.Callback):
    ...
    def on_validation_end(self, trainer, pl_module):
        ...
        output_ranks = log['outputs']
        image_fullpaths = log['input_srcs']

        output_classranks = np.max(output_ranks, axis=1)
        ...
        assert output_ranks.shape[0] == len(log['input_classes']), 'wrong number inputs-to-outputs'
        assert output_ranks.shape[1] == len(class_labels), 'wrong number of class labels'
        ...
        if 'output_classranks' in self.series: results['output_classranks'] = output_classranks
    ...
    def _save_validation_results_json(self,outfile,results):
        for np_series in ['input_classes','output_classes','output_ranks','output_classranks','confusion_matrix']:
    ...
    def _save_validation_results_mat(self,outfile,results):
        ...
        for float_series in ['output_ranks', 'output_classranks', 'confusion_matrix']:
    ...
    def _save_validation_results_hdf(self,outfile,results):
        with h5.File(outfile, 'w') as f:
            ...
            for num_series in ['input_classes','output_classes','output_ranks', 'output_classranks', 'confusion_matrix']:
...
class SaveRunResults(ptl.callbacks.base.Callback):
    ...
    def on_test_end(self, trainer, pl_module):
        ...
        output_ranks = rr.outputs

        output_classranks = np.max(output_ranks, axis=1)
        output_classes = np.argmax(output_ranks, axis=1)

        assert output_ranks.shape[0] == len(output_classes), 'wrong number inputs-to-outputs'
        assert output_ranks.shape[1] == len(class_labels), 'wrong number of class labels'
    ...
    def _save_run_results_json(self, outfile, results):
        ...
        #          input_images output_classes output_ranks

        output = dict(model_id = results['model_id'],
                      timestamp = results['timestamp'],
                      class_labels = results['class_labels'],
                      output_ranks = results['output_ranks'].tolist(),
    ...
    def _save_run_results_mat(self, outfile, results):
        ...
        output['output_ranks'] = results['output_ranks'].astype('f4')
    ...
    def _save_run_results_hdf(self, outfile, results):
        ...
        with h5.File(outfile, 'w') as f:
            ...
            f.create_dataset('output_ranks', data=results['output_ranks'], compression='gzip', dtype='float16')
